...
class curriculum2 extends StatefulWidget {

  final int id;
  final String title;
  final List<Widget> children;

  curriculum2({required this.id,required this.title,required this.children});


  @override
  _curriculum2State createState() => _curriculum2State();
}


class _curriculum2State extends State<curriculum2> {
  List<Item> _data = [];
  ...
  Future<List<Subtopic>> fetchSubtopicsForTopic(int topicId) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${MyConstants.baseUrl}/course_api/curriculum_data/${widget.id}/$topicId'),
        headers: {
          'Authorization': '$accessToken',
        },
      );
                        ...
                        children:  topic.subtopics.map<Widget>((subtopic) {
                          return
                            ListTile(
                                ...
                                title:Row(
                                  children: [Expanded(child: Text(subtopic.name,style: TextStyle(fontWeight: FontWeight.w500),),),
                                    // GestureDetector(
                                    //   onTap: () {
                                    ...
                                    //     }
                                    //   },
                                    //   child: userPurchased && courseMode == 'recorded'
                                    //       ? Icon(Icons.video_library_outlined, color: Colors.black) // Video icon
